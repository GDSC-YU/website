        ...
        <SplideSlide class="flex h-96 justify-center">
            <div class="w-full sm:w-1/2">
                <div class="group relative flex h-full overflow-hidden rounded-2xl shadow-md hover:shadow-2xl">
                    <img src={event.image} alt={event.imageAlt} class="absolute inset-0 h-full w-full object-cover object-center group-hover:blur" />

                    <!-- Hover Content -->
                    <div class="relative z-10 flex w-full flex-col items-center justify-center gap-y-10 px-6 py-14 text-center text-slate-100 opacity-0 transition duration-100 ease-in-out hover:opacity-100">
                        <div>
                            <h1 class="text-xl font-bold tracking-widest text-google-blue md:text-2xl">{event.title}</h1>
